    ...
    <div class="flex flex-col xl:flex-row gap-8 h-full pb-6 w-full">
        ...
        <div class="w-full xl:w-5/12 flex-shrink-0">
            <div class="flex items-center gap-4 mb-6">
                <a href="/yonetim" class="text-xs font-mono uppercase opacity-60 hover:opacity-100 transition-opacity">← Geri</a>
            ...
            <form action="/update" method="POST" enctype="multipart/form-data" class="border-2 border-[var(--text-main)] p-6 shadow-[8px_8px_0px_#A31D1D] bg-[var(--bg-paper)] space-y-4 max-h-[calc(100vh-170px)] overflow-y-auto custom-scrollbar">
                <?= Csrf::field() ?>
                ...
                <div class="border-2 border-[var(--line-color)] p-2 bg-white relative">
                    ...
                    <div class="mt-2 text-xs font-mono opacity-60">Mevcut görsel — yeni görsel yüklenirse değiştirilir.</div>
                </div>
                <?php endif; ?>

                <!-- Görsel Yükle (Değiştirme için boş bırakılabilir) -->
                <div class="space-y-2">
                    <label class="block font-mono text-[10px] uppercase opacity-60 mb-1">Görsel Değiştir (Opsiyonel)</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="title_tr" id="title-tr-input" class="brutalist-input font-bold text-sm" placeholder="ÖRN: Dolunay Altında" required autocomplete="off" value="<?= htmlspecialchars($item['title_tr']) ?>">
                    </div>
                    <div>
                        <label class="block font-mono text-[10px] uppercase opacity-60 mb-1">Başlık (EN)</label>
                        <input type="text" name="title_en" class="brutalist-input font-bold text-sm" placeholder="ÖRN: Under the Full Moon" autocomplete="off" value="<?= htmlspecialchars($item['title_en'] ?? '') ?>">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <textarea name="description_en" rows="3" class="brutalist-input text-xs" placeholder="English description here..." autocomplete="off"><?= htmlspecialchars($item['description_en'] ?? '') ?></textarea>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="type" class="brutalist-input text-xs" required>
                            <option value="photo" <?= ($item['type'] ?? 'photo') === 'photo' ? 'selected' : '' ?>>Fotoğraf (Photo)</option>
                            <option value="drawing" <?= ($item['type'] ?? '') === 'drawing' ? 'selected' : '' ?>>Çizim (Drawing)</option>
                        </select>
                    </div>
                    <div>
