...
if (strpos($current_uri, '/admin/create-patch') !== false) {
    ...
    $status_color = "bg-[var(--text-accent)]";
} elseif (strpos($current_uri, '/admin/galeri') !== false) {
...
<aside class="w-20 md:w-72 bg-[var(--bg-paper)] border-r-2 border-[var(--text-main)] flex flex-col z-20 flex-shrink-0 transition-all duration-300 h-screen">
    ...
    <nav class="flex-1 overflow-y-auto py-8 flex flex-col gap-2">
        ...
        <a href="/admin/add-note" class="nav-item <?php echo (stripos($_SERVER['REQUEST_URI'], 'add-note') !== false || stripos($_SERVER['REQUEST_URI'], 'edit-note') !== false) ? 'active' : ''; ?> py-4 px-8 font-bold uppercase tracking-widest text-sm flex items-center gap-3">
            <span class="text-lg">🗄</span> <span class="hidden md:inline">Notlar</span>
        </a>
        <a href="/admin/galeri" class="nav-item <?php echo strpos($current_uri, '/admin/galeri') !== false ? 'active' : ''; ?> py-4 px-8 font-bold uppercase tracking-widest text-sm flex items-center gap-3">
