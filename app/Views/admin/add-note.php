    ...
    <div class="h-full flex flex-col">
        ...
        <div class="flex flex-col lg:flex-row gap-12 flex-1 min-h-0 h-[calc(100vh-180px)]">
            ...
            <div class="lg:w-2/5 flex flex-col h-full">
                ...
                <form action="/admin/store-note" method="POST" enctype="multipart/form-data" class="border-2 border-[var(--text-main)] bg-[var(--bg-paper)] p-6 shadow-[8px_8px_0px_#A31D1D] space-y-6 flex-1 overflow-y-auto custom-scrollbar">
                    <?= Csrf::field() ?>
                    ...
                    <?php if(!empty($notes)): foreach($notes as $note): ?>
                        <div class="bg-[var(--bg-paper)] border-2 border-[var(--text-main)] p-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:bg-[var(--bg-secondary)] transition-colors">
                            ...
                            <div class="flex gap-2 w-full md:w-auto">
                                <a href="/admin/edit-note?id=<?php echo $note['id']; ?>" class="flex-1 md:flex-none text-center px-3 py-2 bg-[var(--bg-secondary)] border-2 border-[var(--text-main)] text-[var(--text-main)] text-[10px] font-bold font-mono hover:bg-[var(--text-main)] hover:text-[#FEF9E1] transition-colors">
                                    DÜZENLE
                                </a>
                                ...
                                <form method="POST" action="/admin/delete-note?id=<?php echo $note['id']; ?>" onsubmit="return confirm('Bu notu ve PDF dosyasını kalıcı olarak silmek istediğinize emin misiniz?');" class="flex-1 md:flex-none">
                                    <?= Csrf::field() ?>
